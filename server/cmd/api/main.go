...
func runServer() {
	...
	activityRepo := repository.NewDeviceActivityRepository(db)
	inventoryRepo := repository.NewInventoryRepository(db, activityRepo)
	...
	deviceHandler := handler.NewDeviceHandler(deviceSvc, auditLogger, activityRepo)
